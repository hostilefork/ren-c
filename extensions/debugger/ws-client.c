...
// run chrome with websockets: /opt/google/chrome/chrome --remote-debugging-port=9222
...
static int interrupted, port = 7681, ssl_connection = LCCSCF_ALLOW_INSECURE /*LCCSCF_USE_SSL*/;
...
static const lws_retry_bo_t retry = {
    backoff_ms,  /* retry_ms_table (base delay in ms) */
    LWS_ARRAY_SIZE(backoff_ms),  /* retry_ms_table_count (entries in table) */
    LWS_ARRAY_SIZE(backoff_ms),  /* conceal_count (max retries to conceal) */
    3,  /* secs_since_valid_ping (force PINGs after secs idle) */
    10,  /* secs_since_valid_hangup (hangup after secs idle) */
    20  /* jitter_percent (additional random jitter) */
};
         ...
         void *user, void *in, size_t len)
{
    ...
    return callback_dumb_increment(wsi, reason, user, in, len);
