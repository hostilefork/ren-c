...
{
    struct msg *msg = cast(struct msg*, _msg);
              ...
              void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_PROTOCOL_INIT:
        vhd = cast(struct vhd_minimal_server_echo*,
    ...
    case LWS_CALLBACK_SERVER_WRITEABLE:
        ...
        pmsg = cast(struct msg*, lws_ring_get_element(pss->ring, &pss->tail));
        if (!pmsg) {
            lwsl_user(" (nothing in ring)\n");
            break;
        }

        // Note: Definition of `enum lws_write_protocol` is not a true enum,
        // it includes flags; bad design.  Have to cast this to get type
        // correctness in the lws_write() call for the "enum" argument.
        //
        ...
        m = lws_write(wsi, ((unsigned char *)pmsg->payload) +
                  LWS_PRE, pmsg->len, cast(enum lws_write_protocol, flags));
        if (m < (int)pmsg->len) {
