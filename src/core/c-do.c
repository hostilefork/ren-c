...
bool Pushed_Continuation(
    ...
    Flags flags,  // FORCE_HEAVY, etc. for pushed levels
    ...
    if (Is_Group(branch)) {  // [2] for @(gr o up)
        assert(flags & EVAL_EXECUTOR_FLAG_FORCE_HEAVY);  // needed
        require (
          Level* grouper = Make_Level_At_Core(
            &Group_Branch_Executor,  // evaluates to synthesize branch
            cast(Element*, branch),
            binding,
            (flags & (~ EVAL_EXECUTOR_FLAG_FORCE_HEAVY))
        ));
  ...
  switch_on_sigil: {
    ...
    switch (opt Type_Of(branch)) {
      ...
      case TYPE_QUASIFORM:
        ...
        if (flags & EVAL_EXECUTOR_FLAG_FORCE_HEAVY)
