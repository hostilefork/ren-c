...
Result(None) Get_Word(
    ...
    assert(word != SCRATCH);  // need to put bound word in scratch

    // save scratch

    Flags scratch_protect_flag = SCRATCH->header.bits & CELL_FLAG_PROTECTED;
    SCRATCH->header.bits &= ~CELL_FLAG_PROTECTED;

    bool scratch_was_erased = Is_Cell_Erased(SCRATCH);
    if (not scratch_was_erased)
        Blit_Cell(PUSH(), SCRATCH);  // save
    Derelativize(
        SCRATCH,
        word,
        context  // L_binding breaks here [2]
    );

    // save spare

    Flags spare_protect_flag = SPARE->header.bits & CELL_FLAG_PROTECTED;
    SPARE->header.bits &= ~CELL_FLAG_PROTECTED;

    bool spare_was_erased = Is_Cell_Erased(SPARE);
    if (not spare_was_erased)
        Blit_Cell(PUSH(), SPARE);

    // maybe save out

    possibly(out == OUT);
    bool out_was_erased = Is_Cell_Erased(OUT);
    if (out != OUT) {
        if (not out_was_erased)
            Blit_Cell(PUSH(), OUT);
    }
    ...
    if (spare_was_erased)
      ...
      #if DEBUG_POISON_UNINITIALIZED_CELLS
        Force_Poison_Cell(SPARE);
      #endif
        Blit_Cell(SPARE, TOP);
        DROP();
    }
    SPARE->header.bits |= spare_protect_flag;

    if (scratch_was_erased)
        Erase_Cell(SCRATCH);
    else {
      #if DEBUG_POISON_UNINITIALIZED_CELLS
        Force_Poison_Cell(SCRATCH);
      #endif
        Blit_Cell(SCRATCH, TOP_ELEMENT);
        DROP();
    }
    SCRATCH->header.bits |= scratch_protect_flag;
...
Result(None) Get_Path_Push_Refinements(Level* level_)
{
 ...
 detect_path_compression: {
    ...
    if (Is_Group(at)) {
        ...
        Get_Word(OUT, at, binding) except (e) {
            goto return_error;
        }
