...
//    to be articulated as to what the best behavior is.
{
    ...
    Option(const Element*) rest =
        Is_Null_Signifying_Unspecialized(LOCAL(REST))
            ? nullptr
            : Element_ARG(REST);
    ...
    Option(Element*) delimiter =
        Is_Null_Signifying_Unspecialized(LOCAL(WITH))
            ? nullptr
            : Element_ARG(WITH);
    if (delimiter)
  ...
  //    review the idea's relevance...
    ...
    if (Is_Pinned(item) and not Is_Pinned_Blank(item)) {  // fetch and mold
        ...
        if (item_heart == TYPE_WORD or item_heart == TYPE_TUPLE) {
            ...
            Bind_Cell_If_Unbound(word_or_tuple, Level_Binding(sub));
