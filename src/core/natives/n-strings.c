...
//    to be articulated as to what the best behavior is.
{
    ...
    Option(const Element*) rest =
        Is_Nulled_Signifying_Unspecialized(LOCAL(REST))
            ? nullptr
            : Element_ARG(REST);
    ...
    Option(Element*) delimiter =
        Is_Nulled_Signifying_Unspecialized(LOCAL(WITH))
            ? nullptr
            : Element_ARG(WITH);
    if (delimiter)
  ...
  //    review the idea's relevance...
    ...
    if (Is_Pinned(item) and not Is_Pinned_Comma(item)) {  // fetch and mold
