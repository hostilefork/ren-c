...
REBINT Compare_UTF8(const Byte* s1, const Byte* s2, Size l2)
{
    ...
    for (; l1 > 0 && l2 > 0; s1++, s2++, l1--, l2--) {
        ...
        c2 = *s2;
        if (c1 > 127) {
            assume (  // UTF8 good, should use Back_Scan_Utf8_Char_Unchecked()!
              c1 = Back_Scan_Utf8_Char(&s1, &l1)
            );
        }
        if (c2 > 127) {
            assume (  // UTF8 good, should use Back_Scan_Utf8_Char_Unchecked()!
              c2 = Back_Scan_Utf8_Char(&s2, &l2)
            );
        }
        if (c1 != c2) {
...
REBINT Find_Binstr_In_Binstr(
    ...
    const Stable* binstr1,
    REBLEN end1,  // end binstr1 *index* (not a limiting *length*)
    const Stable* binstr2,  // pattern to be found
