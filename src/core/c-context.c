...
Init(Slot) Append_To_Sea_Core(
    ...
    Patch* patch;
    if (id and i_cast(int, id) < MAX_SYM_LIB_PREMADE) {
        patch = &g_lib_patches[i_cast(int, id)];  // pre-allocated at boot [1]
...
VarList* Make_Varlist_Detect_Managed(
  ...
  //=//// COPY INHERITED VALUES FROM PARENT, OR INIT TO TRASH ///////////=//
    ...
    for (i = 1; i <= len; ++i, ++var)  // 0th item is rootvar, already filled
        Init_Void_Signifying_Unset(var);  // need all slots valid before Read_Slot()?
