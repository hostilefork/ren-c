...
for-each-api [
    ...
    let return-keyword: all [return-type != "void", "return "]
 ...
 *
 * As a sort of guiding principle against complexity, Rebol has tried as hard
 * as possible to be able to build and run as pure ANSI C89.  The need for
 * variadic macros, '//'-style comments, and declaring variables midstream in
 * functions meant C99 was ultimately required.
 *
 ...
 *
 * The first parameter is the environment where code will be looked up, which
 * defaults to running in an isolated context inheriting from LIB if null.
 * Note that APIs with a fixed number of parameters (such as rebI()) do not
 * have a distinct `_c89` version.
 *
 * This API is useful even if you aren't using C89, in the case of wanting
 * to put #ifdefs inside of the call... which you aren't allowed to do when
 * the arguments are being gathered by a __VA_ARGS__ macro.
 */
 ...
 *
 * Some features are enhanced by the presence of a C++11 compiler or above.
 * This includes allowing `int` or `std::string` parameters to APIs instead
 * of using `rebI()` or `rebT()`.  These are on by default if the compiler is
 * capable, but can be suppressed.
 *
 * (Note that there's still some conditional code based on __cplusplus even
 * when this switch is used, but not in a way that affects the runtime
 * behavior uniquely beyond what C99 would do.)
 */
 ...
 *
 * The goal is to make it possible that the only include file one needs to
 * make a simple Rebol library client is `#include "rebol.h"`.  Accomplishing
 * that requires including a few headers.
 *
 ...
 * symbols from the executable that loads it.  What has to happen instead is
 * that the EXE passes a table of function pointers to the DLL's init routine.
 * (See RebolApiTable)
 *
 ...
 *
 * We'd like to give a targeted error message if the feature is missing, vs.
 * have the compiler spew out gibberish that is harder to decipher.  But since
 * some pre-C99 compilers actually support __VA_ARGS__, don't use the
 * compiler version to preemptively error.  Instead, try to call a dummy
 * variadic macro...which will lead users to this spot if it does not work.
 */
...
/*
 * The API can be used by the core on value cell pointers that are in stable
 * locations guarded by GC (e.g. frame argument or output cells).  Since the
 * core uses Value*, it must be accurate (not just a void*)
 */
 ...
 *
 * The functions that are named rebXXX with XXX in all caps are designed for
 * passing in variadic argument streams.  So things like rebRELEASING() a.k.a.
 * rebR(), or rebQUOTING() a.k.a. rebQ().  They return "instructions" that are
 * not RebolValue*, and are only for use in the variadic calls.
 *
 * You should not cache references to them in variables, but only pass them
 * directly in the variadic stream.  This is because the feeding of the
 * va_list in case of error is the only way they are cleaned up.
 *
 * They are named with "Internal" to follow the naming convention of
 * "_internal" for things you should not be using in client code.  It could
 * be "RebolBase_internal" though that looks less like a data type in the
 ...
 *
 * When the user writes `rebInteger(i)` it needs a macro to wrap it despite
 * not being variadic.  Because if it's being compiled by the core that needs
 * to resolve as:
 *
 ...
 *
 * These shorthand macros make the API somewhat more readable, but as they
 * are macros you can redefine them to other definitions if you want.
 *
 ...
 * (The C portion of the Emscripten build can use these internally, as the
 * implementation is C.  But when calling the lib from JaveScript, it is
 * obviously not reading this generated header file!)
 *
 * Note: Must be defined before variadic helpers so that the C++ `to_rebarg()`
 * converters for recursive variadic destructuring can use them.
 */
 ...
 *
 * It is very low-level, has no typechecking, and does not offer a count of
 * the number of arguments passed.  So something needs to be passed that
 * signals the end.
 *
 ...
 * implementation code enough to not be worth it.
 */
 ...
 * using variadic templates, allowing each argument to do typechecking (vs.
 * the completely type-unsafe va_list of C).
 *
 * As an added bonus, the processing of the arguments at compile time permits
 * arbitrary transformations.  This means things like `int` can be turned
 * into rebI(...) to produce a RebolValue, instead of an invalid type error.
 *
 * The following converters are predefined, but you can add your own, like
 ...
 *
 * (It's not predefined to avoid forcing inclusion of <string>, but it is easy
 * to add if you want it.)
 *
 * 1. If we didn't explicitly delete the conversion of const void*, then a
 *    nasty corner of C++ is that it would fall back on the bool conversion,
 ...
 *
 * These helpers are called by variadic macros.  Those macros pass parameters
 * they receive with a binding at the beginning and `rebEND` tacked on in the
 * final position.  (See rebEND above for why.)
 *
 * As with non-variadic API entry points, these translate a raw name like
 * `rebValue()` to either `API_rebValue()` or `g_librebol->rebValue()`.  But
 * it also translates the variadic arguments into two pointers called `p` and
 * `vaptr`, that can be passed to the function pointers in the API table
 *
 * If C, then `p` is passed as the pointer to the first element in the
 * variadic sequence (which is actually a fixed argument that's not part of
 * the `...` list).  Then `vaptr` is passed as a pointer to va_list given by
 * va_start().
 *
 ...
 * version is significantly more powerful.  Not only can it type-check the
 * variadic arguments, it can offer hooks to convert C++ values into Rebol
 * values.  (See the Argument Conversion Functions above).
 *
 ...
 * information somehow.  This could be folded into the variadic mechanisms
 * with reb__FILE__(const char*) and reb__LINE__(int) being instructions that
 * debug builds of API client code could slip in.
 *
 * 1. In the spirit of dependency control, we only include <stdarg.h> if
 *    actually using the C variadic method.  This means the API table uses
 *    `void* vaptr` instead of `va_list* vaptr`.
 */
 ...
 * binding context that is relevant.  e.g. when inside a rebFunction(), the
 * context should be for that native's function parameters, chained to the
 * module, then inheriting from lib.
 *
 * Getting this inheritance is tricky.  It means there has to be a global
 * relevant definition for when you're calling a subroutine that's not in a
 * native, and it means the INCLUDE_PARAMS_OF_XXX has to override that same
 * name with a new variable implicating the function.
 *
 * What the name of the variable is needs to vary by module and case, so this
 * does that.  But if you don't specify it at all, then it means the API
 * execution will be done in its own isolated environment that just inherits
 * from lib.
 *
 ...
 *
 *    It's more complex to explain and implement (note that JavaScript doesn't
 *    have pointer-to-pointer support, workarounds are needed).
 *
 *    But the biggest problem is that there's no knowledge in the GC of these
 *    outstanding RebolContext*.  It would take an elaborate scheme to track
 *    the lifetime of such LETs, with additional APIs.  If users are to get
 *    involved, it's better to do that with the already-existing APIs, passing
 *    objects explicitly.
 *
 * 1. rebFunction() is an oddity in that it wants to do reverse its parameters
 *    to put the spec first and the function last, so it can't be variadic or
 *    the C version wouldn't compile-time typecheck the passed in C function.
 *    But it also wants to capture the binding.  Easiest to hardcode it here.
 */
 ...
 *
 * The "C89 API" is really just requiring you to call the C helper functions
 * directly for variadic APIs.  But to make it clearer and shorter to type,
 * the helpers are aliased as `rebXXX_c89`.
 *
 ...
 *
 * (Note that naming the C helpers `rebXXX_c89` in the first place would not
 * give the desired polymorphism of having the wrapping macros work with
 * either C++ or C...unless you named the C++ helpers `rebXXX_c89` as well,
 * which is unnecessarily misleading.)
 */
 ...
 *
 *  2. Memory is freed automatically in the case of a panic in the frame
 *     where the rebAlloc() occured.  This is especially useful when mixing
 *     C code involving allocations with rebValue(), etc.
 *
 *  3. Memory gets counted in Rebol's knowledge of how much memory the system
 *     is using, for the purposes of triggering GC.
 *
 *  4. Out-of-memory errors on allocation automatically trigger panics vs.
 *     needing special handling by returning NULL (which may or may not be
 *     desirable, depending on what you're doing)
 *
 ...
 * rebFree()'d before its frame ends.  To get around this limitation, you can
 * call rebUnmanageMemory() on the pointer...but it will then no longer be
 * cleaned up automatically in case of a panic().
 */
...
/*
 * !!! This is a convenience wrapper over the function that makes a failure
 * code from an OS error ID.  Since rebError_OS() links-in OS specific
 * knowledge to the build, it probably doesn't belong in the core build.
 * But to make things easier it's there for the moment.  Ultimately it should
 * come from a "Windows Extension"/"POSIX extension" or something otherwise.
 *
...
eval wrap overbind (binding of $prep-dir) load3 %prep-libr3-tcc.r

change-dir (join repo-dir %extensions/javascript/tools/)
eval wrap overbind (binding of $prep-dir) load3 %prep-libr3-js.r
